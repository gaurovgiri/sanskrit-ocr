...
for i, cnt in enumerate(contours):
    ...
    peaks, _ = find_peaks(hor_proj, distance=7)
    ...
    for j, peak in enumerate(peaks[:-1]):
        ...
        sub_x, sub_y, sub_w, sub_h = cv2.boundingRect(sub_letter)
        cv2.rectangle(img, (x + peak, y + sub_y), (x + peaks[j+1], y + sub_y + sub_h), (0, 0, 255), 1)

        # Save the sub-letter as an image file
        cv2.imwrite('letters/letter_{}_{}.png'.format(i, j), sub_letter)
